<?php
	namespace Objectify\Objects;
	
	class KnownAttributes
	{
		const GUID___Attribute___Text___Name = "{9153A637-992E-4712-ADF2-B03F0D9EDEA6}";
		public static function get___Text___Name()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Text___Name);
		}
		
		const GUID___Attribute___Text___Value = "{041DD7FD-2D9C-412B-8B9D-D7125C166FE0}";
		public static function get___Text___Value()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Text___Value);
		}
		
		const GUID___Attribute___Text___Content_Type = "{34142FCB-172C-490A-AF03-FF8451D00CAF}";
		public static function get___Text___Content_Type()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Text___Content_Type);
		}

		const GUID___Attribute___Text___CSSValue = "{C0DD4A42-F503-4EB3-8034-7C428B1B8803}";
		public static function get___Text___CSSValue()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Text___CSSValue);
		}

		const GUID___Attribute___Text___CSSClassName = "{0F42BCB6-549A-4407-BB91-DD147443F68F}";
		public static function get___Text___CSSClassName()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Text___CSSClassName);
		}
		
		const GUID___Attribute___Boolean___Render_as_Text = "{15DD9E49-AB6D-44F0-9039-27AF81736406}";
		public static function get___Boolean___Render_as_Text()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Boolean___Render_as_Text);
		}
		
		const GUID___Attribute___Boolean___Singular = "{F1A06573-C447-4D85-B4E7-54A438C4A960}";
		public static function get___Boolean___Singular()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Boolean___Singular);
		}
		
		const GUID___Attribute___Text___Target_URL = "{970F79A0-9EFE-4E7D-9286-9908C6F06A67}";
		public static function get___Text___Target_URL()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Text___Target_URL);
		}
		
		const GUID___Attribute___Text___BackgroundColor = "{B817BE3B-D0AC-4A60-A98A-97F99E96CC89}";
		public static function get___Text___BackgroundColor()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Text___BackgroundColor);
		}
		
		const GUID___Attribute___Text___ForegroundColor = "{BB4B6E0D-D9BA-403D-9E81-93E8F7FB31C8}";
		public static function get___Text___ForegroundColor()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Text___ForegroundColor);
		}
		
		const GUID___Attribute___Date___CreationDate = "{0DD670AF-498D-4FEC-A2CF-20E3E56EB732}";
		public static function get___Date___CreationDate()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Date___CreationDate);
		}
		
		const GUID___Attribute___Boolean___BeginAGroup = "{EC62F6E9-CC14-4330-953B-5267FDF92659}";
		public static function get___Boolean___BeginAGroup()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Boolean___BeginAGroup);
		}
		
		
		const GUID___Attribute___Text___Default_Redirect_URL = "{A1D39058-E934-4DBC-9179-EFAECFDD8B48}";
		public static function get___Text___Default_Redirect_URL()
		{
			return Instance::GetByGlobalIdentifier(KnownAttributes::GUID___Attribute___Text___Default_Redirect_URL);
		}
		
	}
?>